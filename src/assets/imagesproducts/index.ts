...
// The files are the mobile-optimized WebP copies generated by
// scripts/optimize-images.mjs (run automatically through the `prebuild`
// npm script). Keeping the APK small matters: the raw PNGs are ~240 MB.
...
Object.entries(productImagesMap).forEach(([path, module]) => {
  ...
  // path is like "./mobile/e(196).webp" — the original file was "e(196).png".
  // Lookup keys keep the historical ".png" extension so every existing data
  // reference ("e(196).png", "e(1).png", …) keeps resolving after the switch
  // to WebP.
  const fileName = path
    .replace(/^\.\//, '')
    .replace(/^mobile\//, '')
...
if (allAvailableImages.length === 0 && typeof console !== 'undefined') {
  console.warn(
    '[imagesproducts] No optimized images found. Run: node scripts/optimize-images.mjs'
  );
}
